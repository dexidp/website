...
For private clients, the `Authorization` header must contain `<id>:<secret>` (with no trailing return). In this example,
the authorization header can be generated by running the command `echo -n "private-client:app-secret" | base64`
...
For public clients, no secret is included in the `Authorization` header, only the client id and the colon. In this example,
the command `echo -n "public-client:" | base64` generates shown authorization header.
